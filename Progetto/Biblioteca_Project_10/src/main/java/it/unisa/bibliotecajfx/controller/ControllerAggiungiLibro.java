 ...
 * se l'ISBN è già presente, incrementare le copie del libro esistente.
 *
...
public class ControllerAggiungiLibro {
     ...
     * @param listalibri
     * @pre listalibri != null
     * @post Il controller può operare su ListaLibri
     */
    ...
    private void onAggiungiAutore() {
        ...
        // evita duplicati (equals di Autore già confronta nome+cognome)
    ...
    /**
     * @brief Aggiunge un libro alla lista (o incrementa copie se ISBN già presente)
     ...
     * Se l'ISBN esiste già, incrementa le copie del libro esistente e salva su file.
     ...
     * @pre listalibri != null
     * @post Se valido il libro è aggiunto oppure copie incrementate; i dati sono salvati su file
     */
    @FXML
    private void onAggiungiLibro(ActionEvent event) throws IOException {
        ...
        // Campi obbligatori (autore NON è un campo singolo: si usa la lista)
...
if (!titolo.matches(".*\\p{L}.*")) {
    ...
    return;
        ...
        // Se ISBN già presente -> aumenta copie e salva
        if (listalibri.checkISBN(isbn)) {
            ...
            ControllerPopup.showSuccess(txtISBN.getScene().getWindow(),
                    "Il libro è già presente: copie aumentate di 1!");
     ...
     * @post Tutti i campi della form risultano vuoti e la lista autori � resettata
     */
