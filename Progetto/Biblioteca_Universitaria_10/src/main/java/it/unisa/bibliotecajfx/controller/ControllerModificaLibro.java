...
public class ControllerModificaLibro {
    ...
    /**
     * @brief ChoiceBox dove Ë possibile scegliere l'operazione da eseguire (aggiunta/rimozione)
     */
    @FXML 
     ...
     * @param listalibri 
     * @pre listalibri != null
     * @post Il controller pu√≤ salvare la lista su file
     */
...
if (!titolo.chars().anyMatch(Character::isLetter)) {
  ...
  // 6) COPIE: aggiungo, non sostituisco
    ...
    if (delta < 0) {
        ControllerPopup.showError(txtISBN.getScene().getWindow(),
                "La quantit‡ di copie non puÚ essere negativa.");
        return;
    }
   ...
   String isbnVecchio = libro.getISBN();
    if (!isbn.equals(isbnVecchio)) {
        // checkISBN Ë boolean: true => esiste gi‡ (quindi conflitto)
        if (listalibri.checkISBN(isbn)) {
            ControllerPopup.showError(txtISBN.getScene().getWindow(),
                    "Esiste gi‡ un libro con questo ISBN. Non puoi assegnare un ISBN gi‡ presente.");
            return;
        }
    }
 String op = (sceltaOperazione != null) ? sceltaOperazione.getValue() : "Aggiungi";
    ...
    if ("Rimuovi".equals(op)) {
        ...
        if (nuoveCopie < 0) {
            ControllerPopup.showError(txtISBN.getScene().getWindow(),
                    "Non puoi rimuovere pi˘ copie di quelle disponibili.");
            return;
        }
